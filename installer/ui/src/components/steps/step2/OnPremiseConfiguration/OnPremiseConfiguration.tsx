...
export const OnPremiseConfiguration: React.FC<OnPremiseConfigurationProps> = ({ 
  ...
  const canContinue = Boolean(
    ...
    config.controlPlane.sshConfig.password
  );
  ...
  const validateConfig = async (configToValidate?: OnPremiseConfig, yaml?: string) => {
    ...
    try {
      const response = await fetch('/api/onpremise/config/validate', {
        method: 'POST',
  ...
  const testSSHConnections = async () => {
    ...
    try {
      const response = await fetch('/api/onpremise/ssh/validate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        ...
        <Card className="border-zinc-800 bg-zinc-900/50">
          ...
          <CardContent>
            <div className="grid gap-4 text-sm md:grid-cols-5">
              ...
              <div>
                <p className="text-zinc-500">OS Distribution</p>
                <p className="mt-1 font-medium text-white">
                  {(() => {
                    const allNodes = [
                      ...config.controlPlane.nodes,
                      ...config.workerNodes.flatMap(g => g.nodes)
                    ];
                    const ubuntuCount = allNodes.filter(n => n.osType !== 'suse').length;
                    const suseCount = allNodes.filter(n => n.osType === 'suse').length;
                    if (suseCount === 0) return 'All Ubuntu';
                    if (ubuntuCount === 0) return 'All SUSE';
                    return `${ubuntuCount} Ubuntu, ${suseCount} SUSE`;
                  })()}
                </p>
              </div>
            </div>
